...
pub fn part_2(filename: &String) -> i32 {
    println!("Advent of Code 2024, Day 5 [Part 2]");
...
fn retrieve_page(rules: &HashMap<i32, Vec<i32>>, pages: Vec<i32>) -> i32 {
        ...
        if let Some(rule) = rules.get(page) {
            for prev_page in &pages[0..i] {
                if rule.contains(prev_page) {
                    ...
                    pages.sort_by(|x, y| match rules.get(x) {
                        Some(rule) => match rule.contains(y       ) {
                            true => Ordering::Less,
    ...
    debug!("The rule is unbroken an no value is returned");
    return 0;
}

...
pub fn process(input: &str) -> miette::Result<String> {
    ...
    let mut guard = Person {
        position: person.0,
        direction: convert(person.1),
    };

    let mut positions: HashSet<IVec2> =
        HashSet::from([guard.position]);

    while (1..=rows).contains(&guard.position[0])
        && (1..=cols).contains(&guard.position[1])
    {
        let next_position: IVec2 =
            guard.position + guard.direction.get_move();
    ...
    Ok((positions.len()).to_string()) // Subtract the step out of bounds
}
